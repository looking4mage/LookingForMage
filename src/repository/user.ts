...
export async function getByGUID(db : Client, guid: string): Promise<IUser | undefined> {
  const result = await db.query('SELECT email, name, guid FROM public.user WHERE guid = $1', [guid]);

  return result.rows[0];
}

export async function getByEmail(db: Client, email: string): Promise<IUser | undefined> {
  const result = await db.query('SELECT email, name, guid FROM public.user WHERE email = $1', [email]);

  return result.rows[0];
}

export async function verifyUser(db: Client, email: string, password: string): Promise<IUser | undefined> {
  const result = await db.query('SELECT email, name, guid, password FROM public.user WHERE email = $1', [email]);
